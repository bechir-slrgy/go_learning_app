...
All primary keys are **UUIDs** (v4), generated by the database via
`gen_random_uuid()` on insert. A UUID id leaks no row count and no ordering,
and two databases can mint ids without coordinating — unlike the sequential
integers this schema used before.
...
erDiagram
    ...
    tasks {
        uuid        id          PK "gen_random_uuid()"
        uuid        user_id     FK "NOT NULL, ON DELETE CASCADE, the owner"
        text        title          "NOT NULL"
        text        status         "NOT NULL, default 'pending', CHECK pending|submitted|approved|rejected"
        uuid        reviewed_by FK "NULL, ON DELETE SET NULL, the admin who decided"
    ...
    notifications {
        uuid        id         PK "gen_random_uuid()"
        uuid        user_id    FK "NOT NULL, ON DELETE CASCADE, the recipient"
        uuid        task_id    FK "NOT NULL, ON DELETE CASCADE, what it is about"
...
```text
UPDATE tasks SET status='pending' WHERE id='...a-submitted-task-uuid...';
...
A single `DELETE FROM users` can remove rows from all four tables, and no
Go code runs.
...
| users | `users_email_key` | email | UNIQUE — **login looks the user up by email**, and it is the signup conflict check |
| refresh_tokens | `refresh_tokens_pkey` | id | primary key |
| refresh_tokens | `refresh_tokens_token_hash_key` | token_hash | UNIQUE, and refresh/logout look the token up by its hash |
| refresh_tokens | `refresh_tokens_user_idx` | user_id | "log out everywhere" deletes a user's tokens by user_id |
...
| webhooks | `webhooks_pkey` | id | primary key |
| webhooks | `webhooks_user_id_idx` | user_id | delivery looks up a user's hooks |
| notifications | `notifications_pkey` | id | primary key |
...
```powershell
docker exec -i taskdb psql -U taskuser -d tasks -c "\d+ users" -c "\d+ tasks" -c "\d+ webhooks" -c "\d+ notifications" -c "\d+ refresh_tokens"
```
